...
for i in range(len(df)):
    plt.scatter(df[i].hull_length,df[i].p_mc_mag ,facecolor = mycolors[i],label=fnames[i])
plt.legend()
plt.ylim(00,2.1)
plt.xlim(0.,300)
plt.title("length : energy")
plt.show()
...
### Take the largest charge
#ldf = [df[i].groupby('jcount') for i in range(len(df))]
# Quick hack for one case
idx = df[0].groupby(['jcount'])['q_tot_obj'].transform(max) == df[0]['q_tot_obj']

ldf = df[0][idx]
#gldf = ldf[ldf.p_mc_mag<1]
gldf = ldf[ldf.p_mc_mag<0.5]




#for name, group in ldf[0]: 
#    print group
#    print len(group)
#    
#gldf= [ldf[i].groupby('q_tot_obj') for i in range(len(ldf))]


#for i in range(len(gldf)):
plt.scatter(gldf.hull_length,gldf.wpca_0r ,facecolor = mycolors[1],label=fnames[0])
plt.legend()
plt.ylim(0.99,1)
#plt.ylim(0.5,1)
plt.xlim(0.,200)
plt.title("MAX ::::: length : wpca0r")
plt.show()
...
plt.scatter(gldf.hull_length,gldf.wpca_1r ,facecolor = mycolors[1],label=fnames[0])
plt.legend()
plt.ylim(0.,.01)
#plt.ylim(0.5,1)
plt.xlim(0.,200)
plt.title("MAX ::::: length : wpca1r")
plt.show()



plt.scatter(gldf.hull_length,gldf.wpca_2r ,facecolor = mycolors[1],label=fnames[0])
plt.legend()
plt.ylim(0.,.01)
#plt.ylim(0.5,1)
plt.xlim(0.,100)
plt.title("MAX ::::: length : wpca2r")
plt.show()
...
plt.scatter(gldf.hull_length,gldf.hull_area/gldf.hull_length ,facecolor = mycolors[1],label=fnames[0])
plt.legend()
plt.ylim(0,100)
plt.xlim(0,300)
plt.title("MAX :::::  SA /length : length")
plt.show()

plt.scatter(gldf.hull_length,pow(gldf.hull_area,1./2) ,facecolor = mycolors[2],label=fnames[0])
#plt.scatter(gldf.hull_length,pow(gldf.hull_area,2./3) ,facecolor = mycolors[2],label=fnames[0])
plt.legend()
x = [x for x in range(100)]
y = [(1./2)*y-5  for y in range(100)]
plt.plot(x,y,'k--')

plt.ylim(0,300)
plt.xlim(0,100)
plt.title("MAX :::::  SA : length")
plt.show()


plt.scatter(gldf.hull_length,gldf.hull_volume/gldf.hull_length ,facecolor = mycolors[1],label=fnames[0])
plt.legend()
plt.ylim(0,100)
plt.xlim(0,300)
