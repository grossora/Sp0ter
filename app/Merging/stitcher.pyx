...
#------------------------------- #
# --- Shower_Brute_nn 
# ------> compare over all pairs of showers. Forward project distande from average points of smaller shower to larger shower pca 
# --- Track_Stitcher_nn 
# ------>  This algo looks for the closest point between showers. If distance is below thresh then merge.  
# ------>  Also, algo works well with BIRCH dues to small size of N per clusters   
##################################

########################################
# Stiching together showers/tracks  
########################################
...
def Track_Stitcher_nn(dataset,datasetidx_holder,labels, float edge_dist, quick=True):
 ...
 #       print '*****************************************************  size of list  {}'.format(len(points_v))
        min_z = min(points_v)
        max_z = max(points_v)
#        print '+++++++++++++++++++++++++++++++++++++++++++++++++++++  max  of list  {}'.format(max(points_v))
#        print '-----------------------------------------------------  min  of list  {}'.format(min(points_v))
    ...
    for j in range(len(vert_vec)):
        ...
        for k in range(j+1,len(vert_vec)):
            if quick:
                # Coditions to break 
                #if vert_vec[k][2][0] -  < -1.*edge_dist  :# this is not necessarily true... just a way to do a first pass
                #    break
    ...
    for a in range(len(CHQ_vec)):
        ...
        for b in xrange(a+1,len(CHQ_vec)):
            ...
            for i in range(len(first_CHQ[1])):
                for j in range(len(second_CHQ[1])):
                    ...
                    test_dist_sq = (dataset[first_CHQ[1][i]][0] - dataset[second_CHQ[1][j]][0])**2 +(dataset[first_CHQ[1][i]][1] - dataset[second_CHQ[1][j]][1])**2 + (dataset[first_CHQ[1][i]][2] - dataset[second_CHQ[1][j]][2])**2
            ...
            #for i in clst_indexs_a:
                k_dist_sq = (dataset[i][0] -dataset[vp_idx_a][0])**2 +(dataset[i][1] -dataset[vp_idx_a][1])**2 +(dataset[i][2] -dataset[vp_idx_a][2])**2
		...
		# Use the charge weighted PCA
            ...
            for i in datasetidx_holder[clst_label_b]:
                k_dist_sq = (dataset[i][0] -dataset[vp_idx_b][0])**2 +(dataset[i][1] -dataset[vp_idx_b][1])**2  +(dataset[i][2] -dataset[vp_idx_b][2])**2 
	    ...
	    # This is not implemented ..
            ...
            Length_between_vtx = ((vtx_A[0] - vtx_B[0])**2 +  (vtx_A[1] - vtx_B[1])**2 + (vtx_A[2] - vtx_B[2])**2 )**0.5 # This is slow
	    # ^^^ This is the gap
	    ...
	    # or  they are just really really freakin close... 
            if ( (vtx_A[0]- vtx_B[0])**2 + (vtx_A[1]- vtx_B[1])**2 +(vtx_A[2]- vtx_B[2])**2 ) **0.5 < 5:
