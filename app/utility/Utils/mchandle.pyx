...
###########################################################
#---------- Add functions to handle spacecharge  ---------#
###########################################################
...
def mcpart_tshift_sce(pobj_list,tpath):
    ...
    #first correct the spe Find the shift needed
    pishift = spacecharge_shift(pobj_list[1][0],pobj_list[1][1],pobj_list[1][2], tpath)

    # just do x for now
    pobj_list[1][0] = pobj_list[1][0] + (pobj_list[1][3]/1000 +tpc_off )*v_drift- pishift[0]
    # Now fix y and z
    pobj_list[1][1] = pobj_list[1][1] + pishift[1]# for the y
    pobj_list[1][2] = pobj_list[1][2] + pishift[2]# for the z

    for i in range(len(pobj_list[3])):
        gshift = spacecharge_shift(pobj_list[3][i][0][0],pobj_list[3][i][0][1],pobj_list[3][i][0][2], tpath)
        #pishift = spacecharge_shift(pobj_list[3][i][0][0],pobj_list[3][i][0][1],pobj_list[3][i][0][2], tpath)
        # just do x for now
        pobj_list[3][i][0][0] = pobj_list[3][i][0][0] + (pobj_list[3][i][0][3]/1000+ tpc_off )*v_drift - gshift[0]
        #pobj_list[3][i][0][1] = pobj_list[3][i][0][1] - gshift[1]# for the y
        #pobj_list[3][i][0][2] = pobj_list[3][i][0][2] - gshift[2]# for the z
        print 'printing out the gamma'
        print gshift[0]
        print gshift[1]
        print gshift[2]
...
def sce_tshift_point(xyzt,tpath):
    tpc_off = 0.
    v_drift = 0.11143588585879627

    #first correct the spe Find the shift needed
    shift = spacecharge_shift(xyzt[0],xyzt[1],xyzt[2], tpath)

    # just do x for now
    xyzt[0] = xyzt[0] + (xyzt[3]/1000 +tpc_off )*v_drift- shift[0]
    #xyzt[1] = xyzt[1] + shift[1]
    #xyzt[2] = xyzt[2] + shift[2]

    return xyzt 


###########################################################
#-----------     MC Signal defnitions            ---------#
###########################################################

#This requires that the pi0 and the conversion of the daughter photons are in the TPC
def mc_neutron_induced_contained( f ):
    # Adding a hack for time cut with mc effects . this needs to be changed for analysis
...
###########################################################
#---------     MC objects to be used later for viz  ------#
###########################################################
...
def piz_mc_info_2(infile,tpath):
    ...
    for i in tree:
        ...
        ###### Update the points for the SCE #########

        pi0_4vect = sce_tshift_point(pi0_4vect,tpath)
        daughter_4pair[0][0] = sce_tshift_point(daughter_4pair[0][0],tpath)
        daughter_4pair[1][0] = sce_tshift_point(daughter_4pair[1][0],tpath)
